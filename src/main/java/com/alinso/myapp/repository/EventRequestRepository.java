...
public interface EventRequestRepository extends JpaRepository<EventRequest, Long> {
    ...
    @Query("select count(eventRequest) from EventRequest eventRequest " +
            "where  eventRequest.eventRequestStatus=:status and eventRequest.event=:event")
    ...
    @Query("select count(eventRequest) from EventRequest eventRequest where eventRequest.applicant=:user and eventRequest.eventRequestStatus=:approved and eventRequest.result=1")
    Integer findApprovedRequestCountByApplicant(@Param("user") User user, @Param("approved") EventRequestStatus approved);

    @Query("select eventRequest.applicant from EventRequest eventRequest where eventRequest.event=:event")
    List<User> applicantsOfEvent(@Param("event") Event event);

    @Query("select eventRequest.event from EventRequest eventRequest " +
            "where  eventRequest.eventRequestStatus=:status and eventRequest.applicant=:user and eventRequest.result=1 order by eventRequest.createdAt desc")
